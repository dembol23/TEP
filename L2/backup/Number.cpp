...
Number::Number(const Number &other):size(other.size), array(new int [other.size]), isPositive(other.isPositive) {
...
bool Number::isBiggerOrEqual(const Number& other) const {
...
Number Number::divideNumber(const Number& other) const {
    ...
    for (int i = size - 1; i >= 0; i--) {
        ...
        currentDividend = currentDividend.multiplyNumber(Number(BASE));
        ...
        for (int j = BASE - 1; j >= 1 && !q_found; j--) {
            ...
            if (currentDividend.isBiggerOrEqual(product)) {
                q = j;
                currentDividend = currentDividend.subtractNumber(product);
                q_found = true;
            }
        }
        quotient = quotient.multiplyNumber(Number(BASE));
...
Number Number::operator+(const Number& other) const {
    if (isPositive == other.isPositive) { // A + B
        Number result = addNumber(other);
        result.isPositive = isPositive;
        return result;
    }
    if (isBiggerOrEqual(other)) { // A + (-B)
        Number result = subtractNumber(other);
        result.isPositive = isPositive;
        return result;
    }
    Number result = other.subtractNumber(*this); // (-A) + B
